...
- 提供穿越火线、Valorant、CSOL、战地 1/4/5/2042、PUBG 和三角洲行动表现风格。
...
一键生成可转移 ZIP、测试证书 MSIX 和 SHA-256 校验文件（只构建，不会安装）：

```powershell
.\Build-Release.ps1
```

也可以双击 `Build-Release-OneClick.cmd`。单独生成可转移安装包或可选安装器：
...
可转移安装脚本会检查 Xbox Game Bar，备份并修复当前用户的 Game DVR 开关；只有在管理员权限可用时才调整机器级策略和服务启动方式。Game Bar 缺失时会打开 Microsoft Store 页面。构建脚本本身不会修改注册表，只有目标用户运行生成的 `Install-KillConfirm.ps1` 时才执行这些检查。
