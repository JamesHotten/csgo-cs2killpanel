...
async fn is_missing_gsi_candidate(
    ...
    mutable.active_player.initialized
        && mutable.active_observed_player_id.as_deref() == Some(kill.attacker_steamid.as_str())
        && attacker_was_dead
}
...
async fn forward_missing_gsi_kill(app_state: Arc<AppState>, pending: PendingServerKill) {
    ...
    let event = {
        ...
        if mutable.active_observed_player_id.as_deref()
            != Some(pending.kill.attacker_steamid.as_str())
        ...
        if mutable.cs2_local_log_round != mutable.active_player.current_round {
            mutable.cs2_local_log_round = mutable.active_player.current_round;
            mutable.cs2_local_log_round_kills = mutable.active_player.ply_kills;
        ...
        if !controlled_bot_effects_enabled {
            mutable.active_player.has_first_kill_in_round = true;
        ...
        mutable.active_player.crossfire_streak_kills = streak_count;
        mutable.active_player.last_crossfire_kill_at = Some(now);
        ...
        let is_first = !pending.is_last && !mutable.active_player.has_first_kill_in_round;
        ...
        mutable.active_player.has_first_kill_in_round = true;
        mutable.active_player.pending_last_kill = if pending.is_last {
            None
        } else {
        ...
        KillEvent {
            ...
            weapon_name: Some(weapon_display_name(&pending.kill.weapon)),
            money_reward,
            round_number: mutable.active_player.current_round,
            money_epoch: mutable.active_player.money_epoch,
    ...
    app_state.events.publish(event.clone()).await;
