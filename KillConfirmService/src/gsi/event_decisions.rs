{
    ...
    if is_initialized && !steamid.is_empty() {
        ...
        let is_defuse = detect_bomb_defused_action_for(
            player_team,
            current_mode,
            previous_round_bomb_state.as_deref(),
            current_round_bomb_state.as_deref(),
        ...
        if let Some(event_kind) = completed_bomb_action {
            bomb_objective_event_to_send = Some(KillEvent {
                ...
                weapon_name: None,
                money_reward: money_rules::bomb_objective_reward_for(
                    current_mode,
                    economy_version,
                ),
    ...
    if is_initialized && phase_transition_to_over {
        if let (Some(round_data), Some(player_team), Some(win_team)) = (
            ...
            let maximum_ct_elimination_reward = money_rules::ct_elimination_team_reward_for(
                u16::MAX,
                current_mode,
                economy_version,
            );
            let possible_unobserved_ct_elimination_reward =
                maximum_ct_elimination_reward.saturating_sub(known_ct_elimination_reward);
