...
namespace KillConfirmGameBar.Controls.Settings
{
    public sealed partial class RuntimeMaintenancePanel : UserControl
    {
        ...
        private static readonly Uri AudioDevicesUri = new Uri("http://127.0.0.1:3000/audio/devices");
        private static readonly Uri AudioDeviceUri = new Uri("http://127.0.0.1:3000/audio/device");
        private static readonly Uri ShutdownUri = new Uri("http://127.0.0.1:3000/shutdown");
            ...
            try
            {
                ...
                using (HttpResponseMessage response = await client.GetAsync(AudioDevicesUri))
                {
                    ...
                    foreach (IJsonValue value in devices)
                    {
                        ...
                        AudioDeviceSelector.Items.Add(new ComboBoxItem { Content = name, Tag = name });
                    }
        ...
        private static string FormatActiveDevice(string active)
        {
            return string.IsNullOrWhiteSpace(active)
                ? string.Empty
                : Services.LocalizationManager.Text("ActiveOutputPrefix") + active;
        }
        ...
        private async void OnResetPluginClick(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                AppRestartFailureReason restartResult = await CoreApplication.RequestRestartAsync("plugin-reset");
