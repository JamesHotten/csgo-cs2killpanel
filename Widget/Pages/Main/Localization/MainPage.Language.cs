        ...
        private void ApplyCurrentGameGuideLanguage(GameStyleMode style, bool isChinese)
        {
            ...
            switch (style)
            {
                case GameStyleMode.CustomModule:
                    body = isChinese
                        ? "默认提供内置 1～5 杀音效和图标，也兼容 CS2 Customizer 的 1～5 杀结构；图标和语音可以分别创建或导入。缺少当前等级图标时仍使用稳定的内置动画兜底。"
                        : "Includes built-in kill 1-5 audio and icons and supports CS2 Customizer's kill 1-5 structure. Icon and voice packs can be created or imported independently, with the stable built-in animation retained as a missing-level fallback.";
                    voice = isChinese
